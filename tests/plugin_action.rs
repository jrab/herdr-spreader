...
#[test]
fn should_exclude_command_palette_overlay_from_reflowed_layout() {
    let temp_dir = PathBuf::from(env!("CARGO_TARGET_TMPDIR"));
    let config_path = temp_dir.join("plugin-overlay-config.yaml");
    let log_path = temp_dir.join("plugin-overlay-herdr.log");
    let _ = std::fs::remove_file(&log_path);
    std::fs::write(
        &config_path,
        r"workspaces:
...
",
    ...
    let output = Command::new(assert_cmd::cargo::cargo_bin!("herdr-spreader"))
        ...
        .env("HERDR_PANE_ID", "w7:p9")
        .env("FAKE_HERDR_LOG", &log_path)
        .env("FAKE_HERDR_PANE_LABEL", "Command palette")
        .env("FAKE_HERDR_INCLUDE_PALETTE", "1")
        .output()
        .unwrap();
    ...
    assert_eq!(
        ...
        vec![
            ...
            "pane list --workspace w7",
            "pane get w7:p9",
            "pane move w7:p1 --new-tab --workspace w7 --label dev --focus",
            "tab rename w7:t5 dev",
            "pane move w7:p10 --tab w7:t5 --split right --target-pane w7:p1 --ratio 0.5 --no-focus",
            "pane split w7:p10 --direction down --ratio 0.5 --cwd /project --no-focus",
        ]
    );
    assert!(!log.contains("pane move w7:p9"));
    assert!(!log.contains("workspace create"));
